...
static void XderivLn(double x, double *Y, double *dYdx)
{
  ...
  if(y<yeq) *dYdx=0; else 
  {
    ...
    *dYdx=-(Mcdm/x/x)*MPlank*sqrt_gStar*sqrt(M_PI/45)*vSigmaI(x,Beps_,Fast_)*(y-yeq*yeq/y)*sqrt(1+epsY*epsY);;
  }
}
...
{ int i,j,nPrc,nform=log10(1/cut)-2;
  ...
  for(i=0,s=0;i<nPrc;i++)  if(fabs(wPrc[j=srt[i]].weight)>=cut)
  {  s+=wPrc[j=srt[i]].weight;
     if(prcn)
